...
public class BookManager {
	...
	public int getPublisherCount() throws BaseException{
		...
		try {
			conn=DBUtil.getConnection();
			String sql="select count(DISTINCT pubid)cnt from BeanBook ";
			st=conn.createStatement();
			java.sql.ResultSet rs=st.executeQuery(sql);
			if(rs.next())
				return rs.getInt(1);
	...
	public int getNoneBookPublisherCount()throws BaseException{
		...
		try {
			conn=DBUtil.getConnection();
			String sql="select count(DISTINCT pubid)cnt from Beanpublisher where pubid not in(\r\n" + 
					"select DISTINCT pubid from Beanbook) ;";
			st=conn.createStatement();
			java.sql.ResultSet rs=st.executeQuery(sql);
			if(rs.next())
				return rs.getInt(1);
	...
	public double getBookAvgPrice()throws BaseException{
		...
		try {
			conn=DBUtil.getConnection();
			String sql="select avg(price)pri from  Beanbook ;";
			st=conn.createStatement();
			java.sql.ResultSet rs=st.executeQuery(sql);
			if(rs.next())
